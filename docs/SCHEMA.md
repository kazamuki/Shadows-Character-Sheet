...
| `src/styles/fonts.css` | Inter, Roboto Mono, Chakra Petch and Oxanium as embedded WOFF2, generated by `npm run fonts` with their OFL licence in its header; the licence files sit beside it in `fonts/`. Nothing loads from the network (Decision 137) | Generated |
...
137. **The app makes no network request: its fonts are embedded, generated from npm packages, and big numbers get a face of their own.**
     *2026-09-24 · Ken + Claude · Touches: fonts, fonts.css, Inter, Roboto Mono, Chakra Petch, Cerulean Nights, Oxanium, --numeric, --display, stat values, vitals, big numbers, Google Fonts, Fontsource, OFL, offline, file://, blank sheet, build, C7, AQ6*
     - **Decided:** `src/styles/fonts.css` holds each brand font's Latin subset as a WOFF2 data URI, generated by `tools/fonts.mjs` from Fontsource's packages (devDependencies) and committed. The shell links it like any stylesheet, the blank sheet inlines it, and neither built file loads anything from a URL; `build.test.mjs` fails if one does. All three fonts are SIL OFL 1.1, and the licence rides in the CSS header, so the single file carries it. Cerulean Nights, first in the display stack, isn't embedded: it isn't ours to ship, and Chakra Petch is its fallback. The display stack is for headers only. Stat values and the vitals' readouts use `--numeric`, Oxanium (Ken's pick), with fixed-width digits.
     - **Why:** a folder opened offline fell back to system fonts (C7), and every open asked Google for them. About 170 KB buys the same look everywhere (Ken, AQ6). Cerulean Nights on the big numbers read badly: its 8 looks like a 0, so SAN 68 read as 60.
     - **Rejected:**
       - The system-font fallback: the look is part of the product.
       - Loose `.woff2` files: font loading from `file://` varies by browser. A data URI works everywhere.
       - Fetching them once from Google: re-pulls by hand, and no licence text.
       - For the numbers: Roboto Mono (reads as code), Share Tech Mono (too thin), Orbitron (too wide on a phone), Chakra Petch (the headers' face).
     - **Replaces:** nothing.
     - **Revisit if:** a script beyond Latin is needed (a name in Cyrillic, say), or the fonts start to dominate the file's size.
     - **Built:** app 0.26.1. Log 2026-09-24 (S5).

138. **A release is one workflow, dispatched with its version, that creates its own tag.**
     *2026-09-24 · Ken + Claude · Touches: release.yml, deploy-demo.yml, release:check, release:prep, bump, tags, GitHub Release, release notes, Pages deploy, dry run, cloud session, AQ9, R4, R11, C14*
     - **Decided:** `.github/workflows/release.yml`, run from main with a version, checks the release is ready (`npm run release:check`), tests and builds once, creates the `vX.Y.Z` tag and the Release (notes = the version's CHANGELOG section, both HTML files attached), and deploys Pages. A dry run stops after the build, on any branch. A pushed tag still releases. `npm run bump` writes the app version everywhere it's written, and `release:prep` dates the CHANGELOG heading.
     - **Why:** a cloud session can merge but can't push a tag, so each release waited on Ken's machine, and two workflows built the same tag separately (C14). Ken: releasing from the cloud is fine, since the PR process holds (AQ9).
     - **Rejected:**
       - A token Ken provides so a tag push triggers the old pair: a secret to rotate, for the same result.
       - Keeping two workflows with the dispatch added: a tag the workflow's own token makes starts no other workflow, so deploying has to happen in the same run.
       - GitHub's generated notes (a list of PR titles): players already have a section written for them.
       - A `bump` that also moves game data or schema: each needs a judgement (Decision 68) or a `migrate()` step, not a find-and-replace.
