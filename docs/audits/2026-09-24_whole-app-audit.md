...
| **B15** | Hardcore Parkour can never be taken: its prerequisite names an Advantage that doesn't exist | Med | [run] | **closed → F27 → D129** |
...
**Closed 2026-09-24:** the tiers in S2 (Decision 131), STATE's count and live line in S1 (Decision 127), and the scripts in S5 (Decision 138). `npm run bump` writes every version site and the changelog heading, `release:prep` and `release:check` do the release's typing, and a release is one workflow a cloud session can run, so no "vX is live" PR is needed.
...
**Ruled 2026-09-24** (Decision 129, F27 closed). Deighton: Cat Like Balance becomes Acrobatics at rank 4 or better, and Time Sense is removed. Hardcore Parkour now needs 1 Major Milestone, Acrobatics 4 and Danger Sense 1. The stub above never shipped.
...
- **C12: The dev server listens on every interface and doesn't confine paths.** `tools/devserver.mjs` calls `listen(PORT)`, which binds all interfaces, and joins `req.url` onto the repo root without checking the result stays inside. It isn't shipped and is only run by hand, but on shared Wi-Fi it serves any file the user can read. Fix: bind `127.0.0.1` and reject paths outside `ROOT`. *Closed 2026-09-24 (plan S5): both, with encoded traversal tried against it.*
...
- **C14: Releases.** `release.yml` attaches only the character sheet, not the blank sheet, and uses GitHub's auto-generated notes (a list of PR titles) rather than the CHANGELOG section written for players. `release.yml` and `deploy-demo.yml` each build the same tag separately. And the tag has to be pushed from Ken's machine. *Closed 2026-09-24 (plan S5, Decision 138): one workflow, dispatched from main, creates the tag, attaches both files, uses the CHANGELOG section as the notes, and deploys.*
- **C15: The smoke suite is 95% of `verify`'s wall time.** Of about 74 s, `smoke.test.mjs` takes 70 s (59 tests, each booting the 0.9 MB build in jsdom) and `voice.test.mjs` takes 24 s in parallel. Engine, rules, docs and build tests together take under 10 s. There's no quick loop for engine work (R8). *Closed 2026-09-24 (plan S5): `npm run test:fast`, about 13 s.*
