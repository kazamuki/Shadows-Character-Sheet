...
**Pulled 2026-09-22** (all files; the first nine were first pulled 2026-09-04). `020_Synergy_System`, `040_Character_Creation`, `041_Archetypes` and `Magic` re-pulled 2026-09-23 for the TOL edit (Decision 103). `Magic`, `Appendix_Book_of_Known_Spells` and `Appendix_Aberrations` re-pulled 2026-09-24, Scott's finished magic pass. `041_Archetypes`, `043_Advantages`, `044_Disadvantages`, `Magic` and `Appendix_Book_of_Known_Spells` re-pulled again on the evening of 2026-09-24, after Scott's later edits. `040_Character_Creation` and `Gear` followed once they were closed in Word (040 unchanged; Gear's Conditions line now points at Conditions & Recovery). The source renamed two chapters, `046_Gear.docx` and `048_Magic.docx`. The mirror keeps the names `Gear.md` and `Magic.md` because docs and code cite them, so pull from the numbered file. Converted with `pandoc -f docx -t gfm --wrap=preserve`. File names swap the source's spaces and ` - ` for underscores.
